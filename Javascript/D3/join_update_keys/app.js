...
//undefined
add.on("click",function(){quotes=quotes.concat(newQuotes);
d3.select("#quotes").selectAll("li").data(quotes).enter().append("li").text(d=> '"'+d.quote+'-'+d.movie+'('+d.year+')')
.style("margin","20px").style("padding","20px")
add.remove(); });
...
//undefined
add.on("click",function(){quotes=quotes.concat(newQuotes);
listItems.enter().append("li").text(d=> '"'+d.quote+'-'+d.movie+'('+d.year+')')
.style("margin","20px").style("padding","0px").merge(listItems).style("color","red");
add.remove(); });